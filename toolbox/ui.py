        ...
        # Compute the projections
            ...
            for projection, utterance in zip(projections, utterances):
                ...
                self.umap_ax.scatter(projection[0], projection[1], c=[color], marker=mark,
                                     label=label)
                utterance_label = utterance.name.split('\\')[2]
                utterance_labels.append(utterance_label)
            # self.umap_ax.set_title("UMAP projections")
            self.umap_ax.legend(prop={'size': 10})

            # calculate silhouette_score
            if len(set(utterance_labels)) > 1:
                projections_array = np.array(projections)
                s_score = silhouette_score(projections_array, utterance_labels)
                print('silhouette_score', s_score, 'for labels: ', set(utterance_labels))
